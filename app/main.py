...
from meraki_acr_framework.plugin_manager import plugin_manager
from meraki_acr_framework.analyzers.power_supply_down_analyzer import PowerSupplyDownAnalyzer
from meraki_acr_framework.resolvers.power_supply_down_resolver import PowerSupplyDownResolver
...
async def meraki_webhooks(request: Request):
    ...
    try:
        ...
        logger.info(f"Analyzer payload received: {result_payload}")
...
def analyze_event_and_analyzer(event):
    ...
    logger.info("Analyzer: %s", analyzer)
    if analyzer:
